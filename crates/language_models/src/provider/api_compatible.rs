        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new(format!(
                    "To use Gearbox Agent with an {provider_name}-compatible provider, you need to add an API key."
                )))
                .child(
                    div()
                ...
                .child(
                    Label::new(format!(
                        "You can also set the {env_var_name} environment variable and restart Gearbox.",
                    ))
